...
export default async function handler(req, res) {
  ...
  const poolKey = `takes:v15:${sport}:${location.toLowerCase()}${topic ? `:topic:${topic.toLowerCase()}` : ''}`;
  ...
  try {
    ...
    const cityTakes = topic ? await readPool(cache, `takes:v15:${sport}:${location.toLowerCase()}`) : [];
