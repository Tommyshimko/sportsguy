...
4. **Wallet + limits** (`api/_wallet.js`, tables in `qa/2026-09-24-wallets.sql`) - every NEW take is paid for before it is generated and refunded if none comes out: a free take (`FREE_TAKES_PER_DAY`, default 3 per wallet per UTC day, and at most `FREE_DAILY_BUDGET`, default 150, free takes a day across everyone), else a bought one (RevenueCat webhook `api/revenuecat.js` credits them; `api/wallet.js` reads the balance / merges an anonymous wallet on sign-in), else 402. Free takes: 12/hour per IP; paid: 40/hour per wallet. `DAILY_TAKE_LIMIT` (default 1000) is the emergency ceiling on everything. No Supabase = no new takes (fails closed). Only sportsguy.xyz may call it from a browser.
